  ...
  methods: {
    ...
    render: function () {
      ...
      const { report, schedule_data } =
        this.dataSet.games[this.gameIdx];
      ...
      report.shots.home.forEach((shotItem) => {
        ...
        if (shotItem.shot_type === "goal" || shotItem.shot_type === 'emptynetgoal' ) {
    ...
    build: function () {
      try {
        ...
        this.$services.toolService
          .getDataSet(this.groupKey, "widget-final-shots-counter")
          .then((resp) => {
            this.dataSet = resp;
            this.loading = false;
