  ...
  methods: {
    ...
    build: function () {
      try {
        ...
        console.error("[widget-final-spider] getDataSet", e);
        this.loading = false;
      }
    },
    mapTeamData: function (teamName) {
      const obj = {}
      const allowedProps = ['giveaway', 'takeaway', 'hit', 'penalty']
      ...
      Object.entries(teamData).forEach(([playername, playerData]) => {
        playerData.forEach(playerEvent => {
          if(!allowedProps.includes(playerEvent.type)){
            return
          }
...
<template>
  <div :id="id" class="widget scrollable widget-final-spider" :data-cy="$options.name">
